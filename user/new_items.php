...
function seed_menu_items(mysqli $conn, string $shopKey, array $seedItems): void {
    ...
    $stmtCheck = mysqli_prepare($conn, "SELECT COUNT(*) AS cnt FROM menu_items WHERE shop_key = ? AND is_seeded = 1");
    if (!$stmtCheck) {
        return;
    }
...
if (!empty($shop)) {
    seed_menu_items($conn, $shop, $seedMenus[$shop] ?? []);
}
...
$menu = [];
if ($shop !== '') {
    $stmtMenu = mysqli_prepare($conn, "SELECT id, name, price, image, is_available, available_from, available_to, quality_note, auto_ready FROM menu_items WHERE shop_key = ? AND is_seeded = 0 ORDER BY id DESC");
    if ($stmtMenu) {
        mysqli_stmt_bind_param($stmtMenu, "s", $shop);
...
<body>
    <div class="top-nav">
        ...
        <?php if (!empty($shop)): ?>
            <a class="back-link" style="margin-left:8px;background:#2563eb;" href="menu.php?shop=<?= urlencode($shop) ?>">Original Menu</a>
        <?php endif; ?>
    </div>
    <h2><?= htmlspecialchars($shopHeading) ?> NEW ITEMS</h2>
<?php if ($cart_error !== ''): ?>
...
<div class="grid">
<?php if (empty($menu)): ?>
    <div class="card">No new items yet.</div>
<?php else: ?>
